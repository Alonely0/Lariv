...
#[test]
pub fn correctness() {
    let lariv = Lariv::new_with_epoch(10);
    let buf = &lariv;
    let mut r = Mutex::new(Vec::<(LarivIndex<_>, String)>::with_capacity(100));
    let c = &r;
    scope(|s| {
        for i in 1..=330 {
            s.spawn(move || {
    ...
    for (i, _) in r.get_mut().unwrap().iter() {
        assert!(lariv.get_with_epoch(*i).is_none());
        lariv.remove(*i)
    }
}
